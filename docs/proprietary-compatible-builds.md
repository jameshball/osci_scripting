...
`osci_scripting` contains Lua script parsing and script-backed effect support.
...
Scripting should remain optional at the module/product level because it adds a runtime dependency, an API surface for user code, and a sandboxing/security boundary. A proprietary product can include this module only when the dependency, notices, and product security model are intentionally accepted.
